...
const BookingPage = ({ selectedPackage, onClose }) => {
  ...
  const handleToggleAddition = (addition) => {
    const isSelected = selectedAdditions.find((item) => item.id === addition.id);
    if (isSelected) {
      ...
      <div className="bg-[#1B242C] p-6 rounded-lg shadow-lg w-[95%] max-w-[900px] text-white flex flex-col">
        ...
        <div className="grid grid-cols-2 gap-4 mb-4">
          <div>
            ...
            <input type="date" className="w-full p-2 bg-gray-800 text-white rounded-md mt-1" value={date} onChange={(e) => setDate(e.target.value)} />
          </div>
          <div>
            <label className="text-yellow-400">⏰ Time:</label>
            <input type="time" className="w-full p-2 bg-gray-800 text-white rounded-md mt-1" value={time} onChange={(e) => setTime(e.target.value)} />
          </div>
        </div>
        ...
        <div className="bg-[#2A3A45] p-4 rounded-lg mt-4">
          ...
          <div className="grid grid-cols-4 gap-2 mt-2 text-sm">
            {additions.map((addition) => (
              <label key={addition.id} className="flex items-center justify-between bg-[#1F2937] p-2 rounded-md cursor-pointer hover:bg-[#374151]">
                <input type="checkbox" checked={selectedAdditions.some((item) => item.id === addition.id)} onChange={() => handleToggleAddition(addition)} />
          ...
          <div className="flex justify-between mt-4">
            ...
            <button className="bg-red-500 text-white px-4 py-2 rounded-md hover:bg-red-600" onClick={() => alert("Booking Confirmed!")}>
              Confirm Booking
            </button>
          </div>
        </div>
