...
const isSpamComment = (text) => {
  ...
  const symbolPattern = /[^a-zA-Z0-9\s]{3,}/.test(text); 
  const gibberishPattern = /^[bcdfghjklmnpqrstvwxyz]{6,}$/i.test(cleanedText); 

  const badWords = [
    "spam", "scam", "fake", "fraud", "stupid", "idiot", "nonsense",
    "damn", "crap", "hell", "shit", "fuck", "bitch", "bastard",
    "asshole", "retard", "dick", "piss", "slut", "moron", "suck",
    "nigger", "whore", "cunt",
  ];
          ...
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-6 mb-10">
            ...
            <div className="bg-white rounded-lg p-6 shadow-md">
              ...
              <ResponsiveContainer width="100%" height={250}>
                <BarChart
                  data={[
                    { category: "Overall", percent: approved.length ? ((approved.reduce((a, b) => a + (b.rating || 0), 0) / (approved.length * 5)) * 100).toFixed(1) : 0 },
                    { category: "Quality", percent: approved.length ? ((approved.reduce((a, b) => a + (b.serviceQuality || 0), 0) / (approved.length * 5)) * 100).toFixed(1) : 0 },
                    { category: "Response", percent: approved.length ? ((approved.reduce((a, b) => a + (b.responseTime || 0), 0) / (approved.length * 5)) * 100).toFixed(1) : 0 },
                    { category: "Value", percent: approved.length ? ((approved.reduce((a, b) => a + (b.valueForMoney || 0), 0) / (approved.length * 5)) * 100).toFixed(1) : 0 },
                    { category: "Experience", percent: approved.length ? ((approved.reduce((a, b) => a + (b.overallExperience || 0), 0) / (approved.length * 5)) * 100).toFixed(1) : 0 },
                  ]}
